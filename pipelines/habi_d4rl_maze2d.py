...
from cleandiffuser.dataset.d4rl_maze2d_dataset import D4RLMaze2DDataset, D4RLMaze2DTDDataset
...
@hydra.main(config_path="../configs/habi/maze2d", config_name="maze2d", version_base=None)
def pipeline(args):
    ...
    # base config
    dv_base_path = f"release_veteran_d4rl_maze2d_H{args.task.planner_horizon}_Jump{args.task.stride}"
    ...
    # horizon_dataset
    planner_horizon_dataset = D4RLMaze2DDataset(
    ...
    # td_dataset
    planner_td_dataset = D4RLMaze2DTDDataset(d4rl.qlearning_dataset(env), state_normalizer=state_normalizer)
    ...
    if args.mode == "generate_goal_direct_behavior":
        ...
        for i in range(args.num_episodes):
            ...
            while not np.all(cum_done) and t < args.task.max_path_length + 1:
                ...
                ep_reward += finished
                if t % 10 == 9:
                    print(f'[episode{i}, t={t}] finished: {finished}')
